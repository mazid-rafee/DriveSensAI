"""API contract tests for DrowsinessDetection FastAPI server."""
...
os.environ["DROWSINESS_CHECKPOINT_PATH"] = "saved_weights/best_loss.pt"
...
def _valid_request(contract: dict[str, Any], *, mutate: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for index in range(window):
        ...
        # Mild variation so the window is not all zeros-identical noise.
        values[2] = 0.01 * index  # yaw
        values[7] = 0.25  # left_eye_aspect_ratio
        values[8] = 0.25  # right_eye_aspect_ratio
        values[0] = 1.0  # face_detected
        values[5] = 1.0  # left_eye_valid
        values[6] = 1.0  # right_eye_valid
        samples.append(
            {
...
def test_health_returns_model_contract(client: TestClient, model_contract: dict[str, Any]) -> None:
    ...
    assert model_contract["model_version"] == "best_accuracy"
    assert model_contract["feature_count"] == 22
    assert model_contract["window_frames"] == 5
    assert model_contract["sampling_rate_hz"] == 15.0
    assert model_contract["class_names"] == [
        "close",
        "closing",
        "open",
        "opening",
    ...
    assert len(model_contract["feature_names"]) == 22
...
def test_incorrect_feature_count_rejected(
    ...
    payload = _valid_request(model_contract, mutate={"feature_names": names})
    # Also shrink sample widths to match names length so we hit contract check.
